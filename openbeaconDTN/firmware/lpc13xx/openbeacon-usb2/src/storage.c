...
storage_scan_items (void)
{
  ...
  pos = 0;
  while (pos <= (LOGFILE_STORAGE_SIZE - sizeof (msg)))
    {
      storage_read (pos, sizeof (msg), &msg);

      /* verify if block is empty */
      p = (uint8_t *) & msg;
      for (t = 0; t < sizeof (msg); t++)
	if ((*p++) != 0xFF)
	  break;
...
storage_logtxt_fmt (char* buffer, uint32_t index)
{
  ...
  {
    storage_read (index*sizeof(msg), sizeof(msg), &msg);

    if(crc8((uint8_t*)&msg, sizeof(msg)-sizeof(msg.crc)) == msg.crc)
    {
    	prop = msg.prop;
...
static const TDiskFile f_volume_label = {
  ...
  storage_scan_items ();
  /* update file sizes */
  f_logfile.length = g_log_items * sizeof(TLogfileDTNMsg);
