...
static uint16_t oid_last_seen; //destination address
...
main (void)
{
		...
		while (1)
		{
			...
			if (UARTCount)
			{
				...
				for (i = UARTCount; i > 0; i--)
				{
					UARTCount--;
					c = *uart++;
					debug_printf ("Unknown command- please press 'H' for hel\n");

				}

				/* reset UART buffer */
				UARTCount = 0;
				debug_printf ("OUTSIDE\n");
...

	{
			...
			do
			{
				...
				if (ntohs (g_Beacon.pkt.crc) == crc)
				{
					...
					case RFBPROTO_BEACONTRACKER_EXT:
						...
						oid_last_seen = ntohs(g_Beacon.pkt.p.tracker.oid_last_seen);
						//seq = ntohl (g_Beacon.pkt.p.tracker.seq);
						seq = ntohs (g_Beacon.pkt.p.tracker.time);
						break;
					}
		...
		/**********************************************************/
			...
			nRFAPI_SetRxMode (1);
			nRFCMD_CE (1);
			pmu_sleep_ms (1); //Carrier detect
			nRFCMD_CE (0);
			nRFAPI_SetRxMode (0);

			if ((nRFAPI_CarrierDetect () != 0x01) && (g_sequence < 20))
			{
