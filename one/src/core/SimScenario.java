...
import movement.Community;
import movement.EvolvedCommunity;
import movement.GridBasedMovement;
...
public class SimScenario implements Serializable {
	...
	/** Simulate real node connection*/
	public static final String USER_REAL_NODE_S = "useRealNode";
	...
	/**sinks addresses*/
	protected List<DTNHost> sinkAddresses;
	...
	/** Grid used for host movement (if any) */
	...
	protected SimScenario() {
		...
		if (s.contains(USER_REAL_NODE_S))
			this.userRealNode = s.getBoolean(USER_REAL_NODE_S);
		else
			this.userRealNode = false;
		...
		this.world = new World(hosts,worldSizeX, worldSizeY, updateInterval, 
				...
				eqHandler.getEventQueues(),this.userRealNode);
	
		for (DTNHost host : this.hosts)
			host.setNodeDependentEventGeneration(this.world.getNodeDependentEventsQueue());
	...
	/**
	 * Returns the GridMap this scenario uses, or null if scenario doesn't
	 * use any grid
	 * @return SGridMap or null if no map is used
	 */
	...
	protected void createHosts() {
		...
		this.sinkAddresses=new ArrayList<DTNHost>();
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			*/
			...
			} else if (mmProto instanceof GridBasedMovement) {
				this.gridMap = ((GridBasedMovement) mmProto).getMap();
			}/*else if (mmProto instanceof EvolvedCommunity){
			 	this.gridMap = ((Community) mmProto).getMap();
			}*/
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				hosts.add(host);
					...
					if(s.getSetting(ROUTER_S).equals("SinkRouter"))
					sinkAddresses.add(host);
		...
		/*assign the sinkAddresses for all routers*/
		for(DTNHost e:hosts)
			e.getRouter().setSinkAddresses(sinkAddresses);
	}
	...
	public List<DTNHost> getSinkAddresses(){
