...
public class PlayField extends JPanel {
	...
	private MapGraphic gridGraphic;
	private AreaGraphic areaGraphic;
	private boolean showMapGraphic;
	private boolean showAreaGraphic;
	...
	/**
	 * Sets the source for the grid graphics and enables grid graphics showing
	 * @param simMap The map to show
	 */	
	public void setGrid(GridMap gridMap) {
		this.gridGraphic = new MapGraphic(gridMap);
		this.showMapGraphic = true;
	}
	
	/**
	 * Sets the area that in simulation are monitored
	 * @param lowLeft Of the Area
	 * @param topRight Of the area
	 */	
	public void setArea(Coord[] lowLeft_topRight) {
		this.areaGraphic = new AreaGraphic(lowLeft_topRight[0], lowLeft_topRight[1]);
		this.showAreaGraphic = true;
	}
	...
	public void paint(Graphics g) {
		...
		// draw grid (is exists and drawing requested)
		if (gridGraphic != null && showMapGraphic) {
			gridGraphic.draw(g2);
		}
		
		if (areaGraphic != null && showAreaGraphic) {
			areaGraphic.draw(g2);
		}
