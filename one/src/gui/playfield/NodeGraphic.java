...
public class NodeGraphic extends PlayFieldGraphic {
	...
	private static boolean drawHomeContrast = true;
	...
	private static Color hostNameColorAtHome = Color.RED;
	...
	private void drawHost(Graphics2D g2) {
		...
		if (drawCoverage && node.isActive()) {
			...
			for (NetworkInterface ni : interfaces) {
				...
				// my test   g2.setColor(rangeColor);
				g2.setColor(node.color);
				g2.setStroke(NodeGraphic.basicStroke);
				g2.draw(coverage);
			}
		}
		...
		if (drawNodeName) {
			if (this.node.getMovementModel() instanceof Community) {
				if (drawHomeContrast){
					Community comm = (Community) this.node.getMovementModel();
					if (comm.isInsideHome(loc))
						g2.setColor(hostNameColorAtHome);
					else
