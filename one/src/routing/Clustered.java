...
public class Clustered extends DTNRouter {

	/** Router's setting namespace ({@value})*/
	public static final String CLUSTERED_NS = "Clustered";
	public static final int DELTA= 300;  //120
	public static final int GATEWAYOUTDATETIME=400; //120
	...
	/*
	 * with 0.8 probability ratio between total area and covered araea is 0.0025
	 * with 0.2 probability ratio between total area and covered area is 0.04
	 * total average approximate deliver probabiliy is 0.042
	 */
	public static final double DEFAULT_GAMMA = 0.3;//0.042; 
	public static final double DEFAULT_GAMMAHAT = 0.1;//0.014;
	 ...
	 */
	public Clustered(Settings settings) {
		super(settings);
	 ...
	 */
	protected Clustered(Clustered r) {
		super(r);
		this.gamma=r.gamma;
	...
	public void changedConnection(Connection con) {
		...
		if(con.isUp())
			if(!other.isSink() && con.isInitiator(getHost()))
			{
				int currentTime=SimClock.getIntTime();
				Clustered otherRouter = (Clustered)other.getRouter();
				
				if (otherRouter.getClusterId() ==  null)
					return;		
	 ...
	 */
	private void membershipCheck(Clustered otherRouter) {
	...
	private void synch(Clustered otherRouter) {
	...
	void doSynchOf(Clustered otherRouter){
	 ...
	 */
	void doGatewaySynchOf(Clustered otherRouter){
	 ...
	 */
	private void leave(Clustered otherRouter) {
	...
	private void buy(Clustered otherRouter) {
		clusterId=getHost().getAddress()+":"+(seq++);
		getHost().color=new Color(clusterId.hashCode());
		gateways.clear();
...
private void join(Clustered otherRouter) {
 ...
 * 
 */
	boolean checkJoin(Clustered otherRouter){
	...
	private void goToJoin(Clustered otherRouter) {
	...
	private Connection tryOtherMessages() {
		...
		for (Connection con : getConnections()) {
			...
			Clustered othRouter = (Clustered)other.getRouter();
			...
			for (Message m : msgCollection) {
				...
				if (othRouter.hasMessage(m.getId())||m.getHops().contains(other))
					continue;
				if (!clusters.containsKey(m.getTo().getAddress()))
						continue;
				String mCluster = clusters.get(m.getTo().getAddress()).getClusterId();
				
				if(!(clusterId.equals(mCluster))){
					boolean firstCond  = othRouter.getClusterId().equals(mCluster);
					boolean secondCond = gateways.containsKey(mCluster) && (gateways.get(mCluster).getGateway().equals(other.getAddress()));
	...
	public MessageRouter replicate() {
		Clustered r = new Clustered(this);
		return r;
	}
