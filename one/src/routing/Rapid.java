 ...
 * RAPID router
 */
public class Rapid extends DTNRouter {
	...
	private void doHostMapping(Connection con) {
		...
		Rapid otherRouter = ((Rapid) otherHost.getRouter());
	...
	private double getMarginalUtility(Message msg, Connection con, DTNHost host) {
		final Rapid otherRouter = (Rapid) (con.getOtherNode(host).getRouter());
	 ...
	 * @return the current MU value
	 */
	private double getMarginalUtility(Message msg, Rapid router, DTNHost host) {
	...
	private double computeRemainingTime(Message msg) {
		...
		if (delayTable.getDelayEntryByMessageId(msg.getId()) != null) {
			...
			for (Entry<DTNHost, Tuple<Double, Double>> entry : delayTable.getDelayEntryByMessageId(msg.getId()).getDelays()) {
				...
				transferTime = ((Rapid) host.getRouter()).computeTransferTime(msg, msg.getTo()); //MXm(i)	with m element of [0...k]
...
//			Message m = t.getKey();
	...
	private Tuple<Tuple<Message, Connection>, Double> tryOtherMessages() {
		...
		for (Connection con : getConnections()) {
			...
			Rapid otherRouter = (Rapid) other.getRouter();
	...
	@Override
	public Rapid replicate() {
		return new Rapid(this);
	}
